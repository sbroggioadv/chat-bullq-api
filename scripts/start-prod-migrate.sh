...
# Coolify can provide runtime vars through a generated .env file depending on
# the deployment mode. Load it in this shell so migrations and the Node process
...
if [ -z "${DATABASE_URL:-}" ]; then
  echo "ERROR: DATABASE_URL is required before running migrations and starting the API." >&2
  exit 1
fi
